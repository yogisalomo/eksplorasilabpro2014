...
class Model_User {
	...
	public static function sign_up($form) {
		return DB::insert(Model_User::$user_table, array('username', 'password', 'level 3', 'first_name', 'surname', 'address', 'active', 'email', 'hash'))
			->values(array($form['username'], $form['password'], 3, $form['first_name'], $form['surname'], $form['address'], 0, $form['email'], $form['hash']))
			->execute();
	}
	...
	// check whether the username is available
	public static function is_available_by_email_and_hash($email, $hash) {
		if (DB::select()
			->from(Model_User::$user_table)
			->where('email', '=', $email)
			->where('hash', '=', $hash)
	...
	// YANG DITAMBAH xmlrpc_server_add_introspection_data(server, desc)
	public static function verification_step($email, $hash) {
